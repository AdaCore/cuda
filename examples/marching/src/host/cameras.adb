...
package body Cameras is
   ...
   function Create
     (Position : Vector3 := (0.0, 0.0, 0.0);
      ...
      Yaw      : Degree := Default_Yaw;
      Pitch    : Degree := Default_Pitch)
      return Camera_T
   is
      Result : Camera_T;
   begin
   ...
   function Get_View_Matrix (Self : Camera_T) return Matrix4 is
      Result : Matrix4;
   begin
      Init_Lookat_Transform (Self.Position, Self.Position + Self.Front, Self.Up, Result);
      return Result;
   end Get_View_Matrix;

   procedure Update_Camera_Vectors (Self : in out Camera_T) is
   begin
      Self.Front := Normalized
        ((Cos (Single(Radians (Self.Yaw))) * Cos (Single (Radians (Self.Pitch))),
         Sin (Single (Radians (Self.Pitch))),
         Sin (Single (Radians (Self.Yaw))) * Cos (Single (Radians (Self.Pitch)))));
   ...
   procedure Process_Mouse_Movement (Self : in out Camera_T;  X_Offset, Y_Offset : Single) is
   begin
   ...
   procedure Process_Mouse_Scroll (Self : in out Camera_T;  Y_Offset : Single) is
   begin
